...
async function create({ body }) {
  ...
  let line_items = Array.isArray(data.line_items)
    ? data.line_items.map((e) => e)
    : [];

  let product_ids = line_items.map((e) => e.product_id);
  let products = await ProductModel.find({ id: { $in: product_ids } }).lean(
    true
  );
  let order_products = products.map((e) =>
    Object({
      id: e.id,
      title: e.title,
      options: e.options,
  ...
  return {
    error: false,
    order,
    message: `Tạo đơn hàng thành công [${order.id}]`,
  };
}
...
Controller.pay = async function ({ order_id }) {
  ...
  let updated_order = await OrderModel._findOneAndUpdate(
    { id: order_id },
    order_data
  );
  return {
...
Controller.cancel = async function ({ order_id, data }) {
  ...
  if (
    !["customer", "fraud", "inventory", "other"].includes(data.cancel_reason)
  ) {
...
function assertOrderFactory({ OrderModel }) {
  return async function assertOrder({ order_id, order }) {
    ...
    if (order_id) {
      updated_order = await OrderModel._findOneAndUpdate(
        { id: order_id },
        order
      );
    } else {
